...
<p align="center">
  Zotero를 벗어나지 않고 요약, 번역, 메타데이터 확인, 논문 기반 질의응답을 처리한다.
</p>
<p align="center">
...
생성된 요약을 검토하고 번역을 확인하며 처리 메타데이터를 보고 논문에 대해 질문할 수 있게 한다.

PaperFlow는 범용 번역기가 아니다. 학술 논문 읽기 workflow를 위한 **Zotero 네이티브 연구 보조 도구**다.
...
GitHub Releases에서 최신 XPI를 내려받는다.
...
1. Zotero를 연다.
2. **도구 → 플러그인**으로 이동한다.
3. 톱니바퀴 아이콘 → **Install Plugin From File**을 클릭한다.
4. 내려받은 `.xpi` 파일을 선택한다.
5. Zotero를 재시작한다.
6. **Zotero 환경설정** → **PaperFlow** 탭을 연다.
7. Gemini API key를 입력하고 연결 테스트를 실행한다.
...
1. PDF가 첨부된 Zotero 항목을 선택한다(또는 PDF를 직접 선택한다).
2. **도구 → Translate Paper**를 실행한다.
3. 진행 창에 chunk별 상태가 표시된다. × 버튼은 작업을 취소하지 않고 창만 닫으며, 번역은 백그라운드에서
   계속된다.
4. 이전 번역이 있으면 **이어하기**(완료된 chunk 재사용) 또는 **다시 번역**을 선택할 수 있다.

### Reader 사이드바 사용하기

1. Zotero Reader에서 논문을 연다.
2. **PaperFlow** 사이드바 섹션을 연다.
3. **요약**, **번역**, **메타** 뷰를 전환한다.
4. 하단 대화 패널에서 질문한다.
...
선택 카드는 출처를 굵은 제목으로 표시하고 선택한 텍스트를 140자에서 자른다. 전송 전에 × 를 눌러 첨부를
제거할 수 있다. 대화 바깥의 다른 곳을 클릭하면 드래그 선택 칩이 자동으로 사라진다.
...
이 세 artifact는 그 자체로 완결되어 있다. 페이지 렌더와 Figure/Table crop은 번역이 진행되는 동안에만
메모리에 존재한다. 시각 요소는 base64 data URI로 `translated.ko.html`에 포함되므로, 완료된 번역을
표시하는 데 Zotero 외부의 디스크 파일이 전혀 필요하지 않다.
...
`dist/paperflow.xpi`를 Zotero에 설치하고 재시작한 뒤, 오류 콘솔에서 런타임 오류를 확인한다.
...
번역된 구절을 원본 PDF의 해당 span과 정렬한다. 번역된 문장에서 PDF 원문 위치로 이동할 수 있게 한다.

### Zotero 주석 연동

번역되거나 선택된 구절로부터 원본 PDF에 Zotero 하이라이트를 생성한다. 요약과 설명을 Zotero 주석에
연결한다.

### 멀티모달 논문 이해

그림, 표, 수식, 캡션을 설명한다. 시각 요소를 주변 텍스트 및 실험 주장과 연결한다.

### 문헌 조사 workflow

여러 논문을 기여, 방법, 데이터셋, 한계 기준으로 비교한다. 구조화된 읽기 노트를 Markdown, Obsidian,
Notion으로 내보낸다.
...
- PaperFlow는 실험적인 도구다. Zotero 내부 API는 변경될 수 있다.
- 번역과 대화에는 Gemini API 접근 권한이 필요하다.
- AI 출력은 비판적으로 검토해야 한다. 번역과 요약에 오류가 있을 수 있다.
- 아주 긴 논문은 모델의 토큰 한계, rate limit, API 오류에 걸릴 수 있다.
- 레이아웃 인식 번역은 렌더링된 PDF 페이지와 포함된 텍스트를 Gemini로 전송한다. PaperFlow 환경설정에서
  끌 수 있으며, 실패 시 기존 텍스트 전용 파이프라인으로 자동 전환된다.
- Figure/Table 경계는 모델이 판단한 결과이며, 연결된 캡션은 제외하되 탐지된 객체 주위를 보수적으로
  확장한다. 원본 PDF는 변경되지 않는다.
- 로컬 분할 산출물은 Zotero 데이터 디렉터리 아래에 다시 생성되며, 기존 `document.md` 파일은 논문별
  `.backup` 디렉터리에 백업된다.
- 원문 정렬 번역, Zotero 주석 매핑, 멀티모달 이해는 로드맵 항목이며 아직 완전히 구현되지 않았다.

---

## 기여

PaperFlow는 개인 연구 생산성 프로젝트로 시작했지만, 협업은 환영한다.
